...
p = params %>% purrr::map(\(p) {
  ...
  errors = (chords %>% tidyr::unnest(temporal_fractions))$error
  print(plot_error_hist(errors, bins=21, 'major', t_v, 'Temporal Variance'))
  cat("  \n")
                      
  errors = (chords %>% tidyr::unnest(spatial_fractions))$error
  print(plot_error_hist(errors,  bins=21, 'minor', s_v, 'Spatial Variance'))
  cat("  \n")
