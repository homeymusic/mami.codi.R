...
## High Resolution ~ 30,000 Dyads with 10 Harmonics in 15 semitones

```{r, include=F}
high_res = readRDS('./man/data/high_res.rds')
chords <- high_res %>% dplyr::rowwise() %>% dplyr::mutate(
...
core_plots = list(
  ...
  plot_semitone_mami(chords, paste('Major-Minor'),
...
meta_plots = list(
  plot_semitone_colo.cohi(chords, paste('Consonance Low & High'),
                          sigma=sigma,
                          black_vlines=black_vlines,gray_vlines=gray_vlines),
  plot_mami.codi(chords, paste('MaMi.CoDi ~',title), chords_to_label=c()),
...
```{r, fig.height=12, fig.width=12, echo=F, results='asis', message=F}
cat("  \n###", 'High Res ~ Meta Plots', '\n')
...
params %>% purrr::map(\(p) {
  ...
  meta_plots = list(
    ...
    plot_mami.codi(chords, paste('MaMi.CoDi ~',title),
                   chords_to_label=chords %>% dplyr::filter(
                     round(semitone,2) %% 1 == 0
                   )),
    plot_semitone_registers(chords, paste('Ref Freqs & Tonic Timbre'))
  )  
  cat("  \n###", title, '\n')
