...
processing and bricklaying is to estimate ratios (within an acceptable precision)
...
periodicity of chords. precision acts as the physiological limits mentioned by 
Oxenham, above.\

Considering that the spatial and temporal signals had two different physiological
origins, we searched a two-dimensional precision space in order to match model
...
same for temporal and spatial precision. This might indicate that the physiological 
...
within a given precision. How do we find the best precision values? For the 
MaMi.CoDi model we ran thousands of computations with various precision values 
...
we initially did a two-dimensional precision search. However the closest 
fits to the behavioral data came from spatial and precision values being the 
...
p = params %>% purrr::map(\(p) {
  ...
  print(plot_semitone_spatial_temporal(chords, paste('Spatial and Temporal Consonance'),
                                       goal=NULL,sigma=sigma,include_points=F,
