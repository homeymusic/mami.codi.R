...
When we combine the wavelength components of a chord together, we can estimate 
the overall wavelength for the whole chord. The overall chord wavelength will be as long as
...
`r default_tolerance(scale='macro')`.\
...
p = params %>% purrr::map(\(p) {
  ...
  cat("  \n")
  print(plot_semitone_cowave.cofreq(chords, paste('Spatial and Temporal Consonance'),
                                    goal=experiment,sigma=sigma,
                                    black_vlines=black_vlines,gray_vlines=gray_vlines))
  cat("  \n")
})
