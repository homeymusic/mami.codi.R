...
* The dyads immediately below, from the timbre study, are true two-pitch dyads. A 
major third like C4 and E4, for example, only includes C4 and E4.
...
* True two-pitch dyads versus octave-framed, three-pitch dyads, might show
...
params %>% purrr::map(\(p) {
  ...
  print(plot_semitone_mami(chords, paste('Major-Minor'),goal=experiment,
                           include_line=T,sigma=sigma,
                           black_vlines=black_vlines,gray_vlines=gray_vlines))
  print(plot_mami.codi(chords, paste('MaMi.CoDi ~',title), chords_to_label=NULL,
                       include_labels=F))
  print(plot_semitone_colo.cohi(chords, paste('Consonance Frequency (gold) & Wavelength (blue)'),
                                sigma=sigma,
                                black_vlines=black_vlines,gray_vlines=gray_vlines))
  cat("  \n")
})
...
params %>% purrr::map(\(p) {
  ...
  print(plot_semitone_mami(chords, paste('Major-Minor'),goal=experiment,
                           include_line=T,sigma=sigma,
                           black_vlines=black_vlines,gray_vlines=gray_vlines))
  print(plot_mami.codi(chords, paste('MaMi.CoDi ~',title),
                       include_labels=F))
  print(plot_semitone_colo.cohi(chords, paste('Consonance Frequency (gold) & Wavelength (blue)'),
                                sigma=sigma,
                                black_vlines=black_vlines,gray_vlines=gray_vlines))
