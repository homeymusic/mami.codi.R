...
$$M_{\lambda f} = C_{\lambda} - C_{f}$$
Major chords will have positive $M_{\lambda f}$, minor chords negative and neutral chords 0.
...
p = params %>% purrr::map(\(p) {
  ...
  print(plot_error_hist(errors, 'major', t_v, 'Temporal Variance'))
  cat("  \n")

  errors = (chords %>% tidyr::unnest(spatial_fractions))$error
  print(plot_error_hist(errors, 'minor', s_v, 'Spatial Variance'))
  cat("  \n")
