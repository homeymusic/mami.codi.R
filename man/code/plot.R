...
plot_semitone_zscore_sum <- function(chords, title='', include_line=T, sigma=0.2,
                                     include_points=T,
                                     include_linear_regression = F, goal=NULL,
                                     black_vlines=c(),gray_vlines=c()) {
  chords$smoothed.consonance_dissonance = smoothed(chords$semitone,
...
plot_semitone_cowave.cofreq <- function(chords, title='', include_line=T, sigma=0.2,
  ...
  ggplot2::ggplot(chords, ggplot2::aes(x = .data$semitone)) +
    ...
    ggplot2::geom_line(ggplot2::aes(y = .data$smoothed.temporal_consonance), linewidth = 1,
                       color=colors_homey$major) +
    ggplot2::geom_line(ggplot2::aes(y = .data$smoothed.spatial_consonance), linewidth = 1,
...
plot_semitone_codi_wrap <- function(theory, experiment,
  ...
  per_plot_labels$label = per_plot_labels %>%
    purrr::pmap_vec(\(spatial_tolerance) {
      tols = paste0('   tolerance:', spatial_tolerance)
    })
