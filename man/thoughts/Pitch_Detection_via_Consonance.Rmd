...
for (trial in 1:1000) {
  ...
  temporal_scores = looped_paused %>% dplyr::rowwise() %>% dplyr::mutate(
    ...
    distance         = abs(noisy_chord$frequency_consonance-frequency_consonance)
  )
  ...
  spatial_scores = looped_paused %>% dplyr::rowwise() %>% dplyr::mutate(
    paused_frequency = metadata$paused_frequency,
    distance         = abs(noisy_chord$wavelength_consonance-wavelength_consonance)
  )
  ...
  spatiotemporal_scores = looped_paused %>% dplyr::rowwise() %>% dplyr::mutate(
    paused_frequency = metadata$paused_frequency,
    distance         = abs(noisy_chord$consonance_dissonance-consonance_dissonance)
  )
