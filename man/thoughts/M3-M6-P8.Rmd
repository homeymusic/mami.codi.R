...
```{r, echo=F}
homey_M3 = mami.codi.data %>% dplyr::slice(2)
```
...
Cents:
```{r, echo=F}
1200*log(hrep::midi_to_freq(homey_M3$semitone)/hrep::midi_to_freq(60))/log(2)
```

```{r, echo=F}
homey_M3 %>% dplyr::select('semitone', 'consonance_dissonance')
```
