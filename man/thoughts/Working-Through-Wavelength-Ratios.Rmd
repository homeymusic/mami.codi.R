...
```{r, echo=F}
chord$ratios_frequency[[1]] %>% dplyr::arrange(ratio) %>% dplyr::rename(freq=pitch) %>% dplyr::select(-pseudo_ratio)
```
...
```{r, echo=F}
chord$ratios_wavelength[[1]] %>% dplyr::arrange(ratio) %>% dplyr::rename(freq=pitch) %>% dplyr::select(-pseudo_ratio)
```
