...
plot(t,y,type="l", xlab="time", ylab="amplitude_lower_bound", main=paste("A0 ~ L2:", format(round(l2_norm_y, 2), nsmall = 2)))
```
...
y=sin(2*pi*analogy$A0_f*t)
l2_norm_y = sqrt(sum(period*y^2))
plot(t,y,type="l", xlab="time", ylab="amplitude_lower_bound", main=paste("Wiener's Partial A0 ~ L2:", format(round(l2_norm_y, 2), nsmall = 2)))
```
```{r, echo=F, message=F}
y_fft=fft(y)
l2_norm_y_fft = sqrt(sum(((analogy$A0_f+1-analogy$A0_f)*Re(y_fft))^2))
