...
* P0: `r sprintf(chord$P0, fmt = '%#.3f')`

* Quantum Speed of Sound: `r sprintf(chord$P0 * chord$f0, fmt = '%#.3f')`
...
knitr::kable(tibble::tibble(
  ...
  chord_Hz = 1 / chord_s,
  chord_m  = 1 / chord_Hz
) %>% dplyr::select(lcd, chord_Sz, chord_Hz, chord_m, chord_s))
```
...
lcd    = chord$period_alcd
plot_periodicity(period_fractions, lcd, 'period', c_sound = 1, relative = F)
```

##### Chord Wavelength

```{r, echo=F, results='asis', message=F}
plot_periodicity(period_fractions, lcd, 'period', c_sound = 1)
```
...
knitr::kable(tibble::tibble(
  ...
  chord_s  = lcd * max_T_s,
  chord_Hz = 1 / chord_s
) %>% dplyr::select(lcd, chord_Sz, chord_Hz, chord_s))
```
