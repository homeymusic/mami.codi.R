...
knitr::kable(tibble::tibble(
  ...
  lcm_whole      = chord$temporal_lcm_whole,
  chord_Sz = log2(lcm_whole),
  min_f_Hz = min(chord$frequencies[[1]]),
  max_T_s  = 1 / min_f_Hz,
  chord_s  = lcm_whole * max_T_s,
  chord_Hz = 1 / chord_s,
  chord_m  = c_sound / chord_Hz,
  c_sound
) %>% dplyr::select(lcm_whole, chord_Sz, chord_Hz, c_sound, chord_m, chord_s, tol))
```
...
lcm_whole    = chord$temporal_lcm_whole
plot_periodicity(ratios, lcm_whole, 'frequency', relative = F)
```

##### Chord Period

```{r, echo=F, results='asis', message=F}
plot_periodicity(ratios, lcm_whole, 'frequency')
```
...
knitr::kable(tibble::tibble(
  ...
  lcm_whole      = chord$spatial_lcm_whole,
  chord_Sz = log2(lcm_whole),
  max_l_m  = max(chord$wavelengths[[1]]),
  chord_m  = lcm_whole * max_l_m,
...
ratios = chord$spatial_ratios[[1]] %>%
  dplyr::select(index, num, den, tone)
lcm_whole    = chord$spatial_lcm_whole
plot_periodicity(ratios, lcm_whole, 'wavelength', c_sound = c_sound,relative = F)
```

##### Chord Wavelength

```{r, echo=F, results='asis', message=F}
plot_periodicity(ratios, lcm_whole, 'wavelength', c_sound = c_sound)
```
