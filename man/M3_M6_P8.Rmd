...
print(knitr::kable(tibble::tibble_row(
  ...
  spatial_precision        = paste(round(chords$spatial_precision %>% unique, 5), collapse = ' '),
  temporal_precision       = paste(round(chords$temporal_precision %>% unique, 5), collapse = ' '),
)))
...
As the precision value of the MaMi.CoDi model varies from very small to very large
the consonance peaks expand their width until they overcome the dissonance troughs.
See plot sets, below, for M3, M6 and P8.\

MaMi.CoDi's precision value is the only parameter in the model and indicates
...
* Small precisions give more accurate fractions which give longer periods.
* Large precisions give less accurate fractions which give shorter period estimates.

#### M3 ~ Major Third

Plot of M3 with MaMi.CoDi precision values varying from 1e-08 to 0.1:
![M3 with a range of MaMi.CoDi precision values.](./man/figures/precision_search/M3.png)
...
Plot of M6 with MaMi.CoDi precision values varying from 1e-08 to 0.1:
![M6 with a range of MaMi.CoDi precision values.](./man/figures/precision_search/M6.png)
...
Plot of P8 with MaMi.CoDi precision values varying from 1e-08 to 0.1:
![P8 with a range of MaMi.CoDi precision values.](./man/figures/precision_search/P8.png)
