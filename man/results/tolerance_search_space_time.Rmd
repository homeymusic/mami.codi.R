...
theory <- theory %>% dplyr::rowwise() %>% dplyr::mutate(
  ...
  .before=1
) %>% dplyr::group_by(wavelength_tolerance)
```

Tolerances:
```{r, echo=F}
tols = theory$wavelength_tolerance %>% unique
tols
```
...
theory <- theory %>% 
  dplyr::mutate(
    z_score = z_scores(wavelength_consonance),
...
theory <- theory %>% 
  dplyr::mutate(
    z_score = z_scores(frequency_consonance),
    smooth  = smoothed(semitone, .data$z_score, params$sigma)
  )
```
