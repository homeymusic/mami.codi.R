...
knitr::opts_chunk$set(
  collapse = TRUE,
  comment = "#>",
  fig.path = "../figures/precision_search/"
)
...
theory.rds = paste0('../data/precision_',
                    search_label,
                    '.rds')
...
theory <- theory %>% dplyr::rowwise() %>% dplyr::mutate(
  ...
  .before=1
) %>% dplyr::group_by(spatial_precision)
```

```{r, echo=F, results='asis', message=F}
theory <- theory  %>% dplyr::filter(semitone > 0)
experiment <- experiment  %>% dplyr::filter(semitone > 0)
```

Precisions:
```{r, echo=F}
tols = theory$spatial_precision %>% unique
tols
```
...
```{r CoDi, fig.height=7*rows, fig.width=12*cols, echo=F}
...
plot_semitone_codi_wrap(
  theory %>% dplyr::filter(semitone>1), 
  experiment %>% dplyr::filter(semitone>1),
...
```{r Spatial, fig.height=7*rows, fig.width=12*cols, echo=F}
# height = 3 per row and width = 5 per column
# so for grid of 4x12 it would be height=12 and width=60
theory <- theory %>% 
  dplyr::mutate(
    smooth  = smoothed(semitone, spatial_consonance, params$sigma)
  )

plot_semitone_spatial_wrap(
  theory %>% dplyr::filter(semitone>1), 
  gray_vlines  = params$gray_vlines[[1]],
  black_vlines = params$black_vlines[[1]],
  title=paste(search_label),
...
```{r Temporal, fig.height=7*rows, fig.width=12*cols, echo=F}
# height = 3 per row and width = 5 per column
# so for grid of 4x12 it would be height=12 and width=60

theory <- theory %>% 
  dplyr::mutate(
    smooth  = smoothed(semitone, temporal_consonance, params$sigma)
  )

plot_semitone_temporal_wrap(
  theory %>% dplyr::filter(semitone>1), 
  gray_vlines  = params$gray_vlines[[1]],
  black_vlines = params$black_vlines[[1]],
  title=paste(search_label),
