...
knitr::opts_chunk$set(
  collapse = TRUE,
  comment = "#>",
  fig.path = "../figures/sfoae_num_harmonics/"
)
...
theory.rds = paste0('../data/output/sfoae_num_harmonics_',
                    search_label,
                    '.rds')
...
theory <- theory %>% 
  dplyr::rowwise() %>% 
  dplyr::mutate(
    sfoae_num_harmonics = metadata$sfoae_num_harmonics,
    num_harmonics  = metadata$num_harmonics,
    octave_ratio   = metadata$octave_ratio,
    semitone       = metadata$semitone,
    include_beats  = metadata$include_beats,
    .before=1
  ) %>% 
  dplyr::ungroup() %>%  
  dplyr::group_by(sfoae_num_harmonics)
```
...
```{r, echo=F}
sfoae = theory$sfoae_num_harmonics %>% unique
```
...
plot_semitone_codi_sfoae_num_harmonics_wrap(
  theory, 
  experimental_data,
...
plot_semitone_space_time_sfoae_num_harmonics_wrap(
  theory, 
  gray_vlines  = params$gray_vlines[[1]],
  black_vlines = params$black_vlines[[1]],
  title=paste(search_label),
...
```{r _Beating, fig.height=7*rows, fig.width=12*cols, echo=F}
# height = 3 per row and width = 5 per column
# so for grid of 4x12 it would be height=12 and width=60

plot_semitone_beating_sfoae_num_harmonics_wrap(
  theory, 
  gray_vlines  = params$gray_vlines[[1]],
  black_vlines = params$black_vlines[[1]],
  title=paste(search_label),
