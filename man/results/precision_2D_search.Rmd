...
knitr::opts_chunk$set(
  collapse = TRUE,
  comment = "#>",
  fig.path = "../figures/precision_2D_search/"
)
...
theory.rds = paste0('../data/precision_2D_',
                    search_label,
                    '.rds')
...
theory <- theory %>% dplyr::rowwise() %>% dplyr::mutate(
  ...
  .before=1
) %>% dplyr::group_by(temporal_precision, spatial_precision)
```

precisions:
```{r, echo=F}
temp_tols = theory$temporal_precision %>% unique
temp_tols
```

```{r, echo=F}
space_tols = theory$spatial_precision %>% unique
space_tols
```
