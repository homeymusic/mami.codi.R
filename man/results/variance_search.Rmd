...
knitr::opts_chunk$set(
  collapse = TRUE,
  comment = "#>",
  fig.path = "../figures/standard_deviation_search/"
)
...
theory.rds = paste0('../data/standard_deviation_',
                    search_label,
                    '.rds')
...
max_dissonance = max(theory$dissonance)
theory <- theory %>% dplyr::rowwise() %>% dplyr::mutate(
  ...
  consonance     = max_dissonance - dissonance,
  temporal_consonance = max_dissonance - temporal_dissonance,
  spatial_consonance  = max_dissonance - spatial_dissonance,
  .before=1
) %>% dplyr::group_by(temporal_standard_deviation)
```
...
Temporal Variances:
```{r, echo=F}
tols = theory$temporal_standard_deviation %>% unique
tols
```

Spatial Variances:
```{r, echo=F}
tols = theory$spatial_standard_deviation %>% unique
tols
```
Number of Harmonics:
```{r, echo=F}
...
```{r _smoothCoDi, fig.height=7*rows, fig.width=12*cols, echo=F}
...
theory <- theory %>% 
  dplyr::mutate(
    smooth_temporal = smoothed(semitone, max(temporal_dissonance) - temporal_dissonance, params$sigma),
    smooth_spatial  = smoothed(semitone, max(spatial_dissonance) - spatial_dissonance, params$sigma)
  )
