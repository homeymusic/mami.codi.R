---
title: "Standard Deviation Search"
output:
...
theory <- theory %>% 
  ...
  dplyr::mutate(
    ...
    include_time_beats = metadata$include_time_beats, 
    include_space_beats = metadata$include_space_beats,
    .before=1
  ) %>% 
  dplyr::ungroup() %>%  # Ungroup after rowwise operations
  dplyr::group_by(time_standard_deviation, space_standard_deviation)  # Group by both variables
```
...
```{r, echo=F}
tols = theory$time_standard_deviation %>% unique
tols
```

space Standard Deviations:
```{r, echo=F}
tols = theory$space_standard_deviation %>% unique
tols
```

include time beats:
```{r, echo=F}
theory$include_time_beats %>% unique
```

include space beats:
```{r, echo=F}
theory$include_space_beats %>% unique
```
