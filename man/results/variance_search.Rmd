...
theory <- theory %>% dplyr::rowwise() %>% dplyr::mutate(
  ...
  .before=1
) %>% dplyr::group_by(frequency_variance)
```
...
Temporal Variances:
```{r, echo=F}
tols = theory$frequency_variance %>% unique
tols
```

Spatial Variances:
```{r, echo=F}
tols = theory$period_variance %>% unique
tols
```
Number of Harmonics:
```{r, echo=F}
...
```{r _Spatiofrequency, fig.height=7*rows, fig.width=12*cols, echo=F}
# height = 3 per row and width = 5 per column
# so for grid of 4x12 it would be height=12 and width=60

theory <- theory %>% 
  dplyr::mutate(
    smooth_frequency = smoothed(semitone, frequency_consonance, params$sigma),
    smooth_period  = smoothed(semitone, period_consonance, params$sigma)
  )

plot_semitone_period_frequency_wrap(
  theory, 
  gray_vlines  = params$gray_vlines[[1]],
  black_vlines = params$black_vlines[[1]],
  title=paste(search_label),
