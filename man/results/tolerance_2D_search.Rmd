...
theory <- theory %>% dplyr::rowwise() %>% dplyr::mutate(
  ...
  .before=1
) %>% dplyr::group_by(temporal_tolerance, spatial_tolerance)
```

Tolerances:
```{r, echo=F}
temp_tols = theory$temporal_tolerance %>% unique
temp_tols
```

```{r, echo=F}
space_tols = theory$spatial_tolerance %>% unique
space_tols
```
...
```{r, echo=F}
cols = length(space_tols)
rows = length(temp_tols) 
```
