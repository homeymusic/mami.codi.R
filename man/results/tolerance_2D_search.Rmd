...
knitr::opts_chunk$set(
  collapse = TRUE,
  comment = "#>",
  fig.path = "../figures/tolerance_2D_search/"
)
...
theory.rds = paste0('../data/tolerance_2D_',
                    search_label,
                    '.rds')
...
theory <- theory %>% dplyr::rowwise() %>% dplyr::mutate(
  ...
  .before=1
) %>% dplyr::group_by(frequency_tolerance, wavelength_tolerance)
```

Tolerances:
```{r, echo=F}
freq_tols = theory$frequency_tolerance %>% unique
freq_tols
```

```{r, echo=F}
wave_tols = theory$wavelength_tolerance %>% unique
wave_tols
```
...
```{r, echo=F}
cols = length(wave_tols)
rows = length(freq_tols) 
```

## CoDi Wrap

```{r, fig.height=7*rows, fig.width=12*cols, echo=F}
# height = 3 per row and width = 5 per column
# so for grid of 4x12 it would be height=12 and width=60
plot_semitone_codi_grid(
  theory, 
  experiment, 
