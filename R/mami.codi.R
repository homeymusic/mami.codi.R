...
duplex <- function(x, tolerance) {
  ...
  f = x$frequencies[[1]]
  λ = x$wavelengths[[1]]
  pseudo_n = ceiling(x$pseudo_octave ^ log2(ceiling(2 ^ (log(max(f) / min(f)) / log(x$pseudo_octave)))))
  pseudo_n = max(f) / min(f)

  x %>% dplyr::mutate(

    # estimate the frequency cycle
    estimate_cycle(f,
                   max(f) / pseudo_n, # this is the lowest harmonic of the highest tone
                   x$pseudo_octave,
                   tolerance) %>%
    ...
    estimate_cycle(λ,
                   max(λ) / pseudo_n, # this is the highest harmonic of the lowest tone
                   x$pseudo_octave,
                   tolerance) %>%
